...
同日重新核实当前 Codex 后发现，0.146.0 已公开 `codex app-server`：它通过 JSON-RPC 提供 thread、turn、item、status、usage、interrupt、approval 与 request-user-input 等结构化 surface，并可用当前二进制生成版本一致的 TypeScript schema。最初 probe 只验证 initialize、thread/turn、final 与 interrupt，因而没有把 process continuity 纳入首版结论。2026-08-21 复核 committed stable schema并运行 exact `0.146.0` probe 后，进一步确认 sandboxed `command/exec` family 可提供 connection-scoped start、stream output、stdin 与 terminate；status、可重读 output、opaque handle 与隔离仍必须由 agent-nexus owner实现。unsandboxed experimental `process/*` 与 background terminal 继续禁用。
...
同一 live app-server connection 还可以由 backend-private process owner 调用 stable `command/exec` family。该 owner必须提供 opaque handle、本地 status、cursor output、stdin/terminate 串行化与 SessionKey 隔离；turn terminal 不结束 process，connection/daemon restart 则使 handle失效并清理整个 PGID。完整契约由 [`codex-app-server-process.md`](../spec/agent-backends/codex-app-server-process.md) 唯一定义。
...
- app-server 本身、stable `command/exec` process owner 及仍禁用的 experimental surface 会增加 Codex 版本锁定、schema 更新和 release probe 成本。
...
- 以 exact `0.146.0` 的 stable `command/exec` start barrier、跨 turn output/stdin/terminate、隔离与 no-orphan E2E 作为 live process continuity 的 acceptance gate。
...
- 不承诺 daemon crash 后恢复进行中的 turn、pending approval 或 live process；只承诺 supervisor 清理旧 PGID，conversation thread 与 process handle 分离恢复。
- 不把 app-server 的全部公开方法直接暴露给飞书；只有 spec allowlist 内操作可调用。
- 不决定 Claude Code 或其它 CLI 是否存在类似 structured primary。

## Amendments

- 2026-08-21：更正最初把 process control 笼统归为 experimental 的判断。允许 backend-private owner 使用 stable、sandboxed `command/exec` family；继续禁止 unsandboxed experimental `process/*`、background terminal 与通用 passthrough。process 不跨 app-server/daemon restart恢复。
...
- 相关 spec：[`../spec/agent-backends/codex-app-server.md`](../spec/agent-backends/codex-app-server.md)、[`../spec/agent-backends/codex-app-server-process.md`](../spec/agent-backends/codex-app-server-process.md)、[`../spec/agent-runtime.md`](../spec/agent-runtime.md)
