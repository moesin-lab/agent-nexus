---
title: ADR-0011 turn 层级——daemon 视角外显两层，agent 内部留作 backend 细节
type: adr
status: active
...
# ADR-0011：turn 层级——外显两层，agent 内部留给 backend
...
把超时阈值调大、把已有 spec 字段串通到实现。不动外显概念。
...
不只把"用户视角一回合"和"子进程寿命"分开，还把 agent 内部"一次想 + 调工具 + 再想"也升为外显概念——daemon 能感知每一步、按步记账、按步限流、按步取消。
...
- adapter 实现要分别管 backend 进程寿命和事件流推送两件事，不再用"子进程退出 = 一回合完成"这一单一信号搪塞过去
...
- 触发本 ADR 的事故 trace：`1c68898f-eba5-447f-89f6-884102fb8dc9`（IM 60s wallclock timeout）
