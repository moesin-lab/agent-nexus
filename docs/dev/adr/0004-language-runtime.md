...
- monorepo 切分锁定（见 §"采纳后的执行补丁清单" TS-P7）：`@agent-nexus/protocol`（仅类型 + JSON schema 权威源）/ `@agent-nexus/daemon`（daemon 进程整体实现）/ `@agent-nexus/vscode`（extension）/ `@agent-nexus/web`（web frontend）/ `@agent-nexus/cli`（CLI 入口）
...
├── daemon/            # daemon 进程整体实现；包内职责子目录平铺；可独立 npm 启动
