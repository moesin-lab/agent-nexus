...
¹ `status` 原样转写 [`agent-runtime.md`](../agent-runtime.md) §AgentEvent 定义的工具调用终态值，值域 SSOT 见该 owner；日志侧仅记录、不另定义。`cancelled` 终态对应工具被 interrupt / 超时取消（语义独立于单条 `tool_result.isError`，见 agent-runtime owner 说明）。
