...
被用户纠正后，agent 应在完成当前纠正之后**自检一次**：这条经验值不值得沉淀。值得就沉淀，不值得就不动作——**stay quiet by default，不要自动在每次回复末尾附"沉淀建议"**。

### 自检 4 步

1. **真问题吗**：是一次性的当下错误，还是有跨 session 复发风险的模式？只有后者值得沉淀。
2. **沉淀到哪层**：见下方分流矩阵。
3. **走对应执行约束**：仓库内任何文件改动遵守核心原则 1（分支先行）；写 auto-memory 遵守全局 `~/.claude/CLAUDE.md` 的"记忆"节。
4. **复核**：执行前 grep 同主题条目（auto-memory 的 `MEMORY.md` + `docs/dev/process/`），避免重写。

### 分流矩阵

| 经验性质 | 沉淀层 | 是否需 PR |
|---|---|---|
| 用户个人偏好 / 协作反馈 / 身份锚点 / 当前项目状态 / 外部系统指针 | auto-memory（各 harness 的 memory 目录） | 否 |
| 项目流程规则 / 编码与错误标准 | `docs/dev/process/` 或 `docs/dev/standards/` | 是 |
...
用户说"沉淀 / 反思 / refine / 复盘 / 记下来"时 agent 必须走自检 4 步。无显式触发时由 agent 自决是否启动自检——**宁可少做不要多做**。
