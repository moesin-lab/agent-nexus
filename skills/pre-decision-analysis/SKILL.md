...
description: 当用户提出需要人类拍板但必须先做结构化分析的问题（评估、对比、讨论、值不值得、该不该、拆不拆、要不要起 ADR）时触发。本文件是 harness-neutral 通用入口；规则权威源在 skill 本目录下 `process/README.md` + `standards/README.md`（symlink 到 docs/dev/）。关键词：决策分析、多方案对比、异构视角反方分析、结构化提问、PR diff review、scratch 协作。
---

# pre-decision-analysis（通用入口）

> ⚠️ **规则权威源**：本 skill 目录下的 `process/`（流程编排 + 子流程）与 `standards/`（产物合格条件 + scratch 模板）。这两个目录是 symlink，物理位置在 `docs/dev/process/pre-decision-analysis/` 与 `docs/dev/standards/pre-decision-analysis/`，受 doc-ownership 体系治理。规则冲突时以 docs 物理位置内容为准。
>
...
- `process/README.md` — 流程编排（主轴 6 步 / 子流程索引 / per-harness 实现）
- 按需加载子流程：`process/subflow-argue.md`（几乎必跑）/ `process/subflow-external-repo.md` / `process/subflow-adr-options.md` / `process/subflow-task-breakdown.md` / `process/subflow-survey.md`
- `standards/README.md` — 触发判据 / 核心原则 / 反模式 / Scratch 硬触发条件
- `standards/scratch-template.md` — scratch 骨架（仅 scratch 硬触发时）

形态决策见 [`docs/dev/adr/0010-pre-decision-agent-first.md`](../../docs/dev/adr/0010-pre-decision-agent-first.md)（不在 skill 目录内——形态 ADR 跨多个潜在 skill 复用）。
...
- **Scratch 协作**：讨论复杂 / 跨会话时起 scratch（路径 C），遵循 `standards/scratch-template.md` 骨架
