...
- 本节列的是**能力声明**，不是可执行指令。若当前 harness 有对应执行器（见上表），agent **必须**按执行器列出的具体工具执行（Claude Code 用 `AskUserQuestion` / `codex-review` 等；其他 harness 用执行器声明的对应工具），不得停在本通用入口的抽象描述上，用普通对话文本 / 手工模拟提问 / 自问自答假 argue 等通用做法代替 harness 特色路径
- 若无对应执行器（fallback 把本文件挂为触发器时；如 Claude Code 的 `scripts/sync-claude-skills.sh`），应按 `docs/dev/process/pre-decision-analysis/` agent-agnostic 权威源 + 本 harness 自身能力兜底；某能力本 harness 做不到时**在 PR 里显式声明缺口**，不装作走了特色路径
