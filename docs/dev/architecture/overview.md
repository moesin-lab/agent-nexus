...
入口 / 中枢 / 适配器三层，**依赖方向单向**（见 [`dependencies.md`](dependencies.md)）：

> **命名 disambiguation**：本节及全文中 `core` / `agent` / `platform` 是**架构分层维度**的概念名，不是 npm package 名。在 ADR-0004 monorepo 物理布局下，三层全部住在 `@agent-nexus/daemon` 这一个 npm package 内（按职责子目录平铺）；`core` 中枢层 = `packages/daemon/src/` 下 engine / session / observability / idempotency / redact / cost / persistence / types 等子目录的集合，**不再立同名 `core/` 子目录**。文档中带点的 namespace prefix（`daemon.logger` / `daemon.idempotency` 等）= `@agent-nexus/daemon` 的 import path；散文中的 "core 层"、"core 是中枢" 等措辞均指分层概念。
