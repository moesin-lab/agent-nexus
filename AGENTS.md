...
**协作性 skill**（影响协作产出格式 / 需他人理解产物 / 多人共用；见 [ADR-0007](docs/dev/adr/0007-collaborative-skill-promotion.md)）入库在仓库根 `skills/`，由 [`skills.manifest`](skills.manifest) 声明。各 harness 的 skill 目录（`.claude/skills/` / `.codex/skills/` 等）按"harness 配置不入库"约定 gitignored；首次 clone 后或 manifest 变更后要把 `skills/` 下的 skill 挂接到本地 harness 目录。

**Claude Code 协作者**（首次 clone 后运行一次）：

```bash
bash scripts/sync-claude-skills.sh
```

幂等 + 清理（详见脚本注释）。或对 `skills.manifest` 每行 `<name>` 手动跑：

```bash
ln -sfn "../../skills/<name>" ".claude/skills/<name>"
```

**其他 harness**（Codex / Cursor / ...）：本 repo 未内置挂接脚本，按上述模式自行挂接到各自配置目录。规则本身以 `docs/dev/process/` 下的权威源为准，skill 只是执行器。

⚠️ **未挂接 → 协作 skill 静默不触发 → 协作产出格式漂移**——且 `.claude/` gitignored，`git status` 看不到异常。clone 后记得跑一次脚本。

纯个人偏好 skill 不在此范围——它们继续留各自 harness 的 local 配置目录，不用入库。
