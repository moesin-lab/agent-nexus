...
本次 epic 要接入 Codex CLI。P1 本机证据已经坐实 `codex-cli 0.133.0` 的可用路径：`codex exec --json` 输出 JSONL，`codex exec resume <thread_id>` 可以续接上下文，`command_execution` 事件能表达 shell 工具开始/完成，`turn.completed.usage` 提供 token 用量，认证/模型错误会落到 `error` 与 `turn.failed`。中断时 Codex 不产 terminal JSONL，runtime 必须合成 `turn_finished{reason:"user_interrupt"}`。

Codex CLI 与 Claude Code CLI 的关键差异是进程模型和安全模型。Claude Code 主路径是长驻 stream-json 子进程，且可通过已验证的 `can_use_tool` control 通道实现执行前工具白名单。Codex P1 只验证了非交互 `exec --json` + `resume`；help 中没有 tool whitelist / allowlist / denylist 语义，安全默认值只能先由 sandbox、approval、working directory、add-dir 与 user config/rules 加载策略表达。
...
- **缺点**：CLI 需要路由两个 backend package；P3-P6 要维护两套 backend contract fixture。
- **主要风险**：如果 config 路由写进 CLI 业务逻辑，会违反 hub-and-spoke 边界；缓解见 spec 中的配置归属约束。

### Option C：先做实验脚本，不接入产品配置

- **是什么**：只保留 GOAL/P1 脚本或 standalone verifier，不新增 runtime package。
- **优点**：短期改动最少。
- **缺点**：不能让 daemon/cli 选择 Codex backend，无法满足 epic 验收；P1 证据不能转化为产品契约。
...
- P3 新增 `packages/agent/codex`，导出 `createCodexRuntime`、`parseCodexConfig`、`CodexConfigError`、`runCompatibilityProbe`。
- P4 runtime 必须覆盖 JSONL fixture、resume、多轮、错误、usage、tool events、interrupt 合成。
- P5 CLI 只按 `agent.backend` 做路由，不在 `packages/cli` 写 Codex 业务逻辑。
- P6 端到端 verifier 必须验证两轮会话、中断和错误路径；认证缺失时给明确 skip/fail reason。

## Out of scope

- 不决定把 Codex 设为默认后端；默认 backend 已在本 ADR 与 spec 中定为 `claudecode`，未来若要调整默认值需另行修订。
- 不决定支持 Codex experimental `exec-server` / `app-server` wire protocol；P1 未验证，默认不用。
- 不决定 Codex 的 native tool whitelist；P1 未发现对应 CLI flag，当前能力声明必须标为不支持。
...
- P1 证据：`/home/node/.goal/codex-agent/drafts/p1-codex-cli-contract-evidence.md`
