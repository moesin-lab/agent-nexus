...
`spikes/` 之外的探针残留，或 PR 同时合入"探针 + 实现"，均不合格。
...
验收流程见 [`../process/tdd.md`](../process/tdd.md)。
...
永远不 skip，永远不注释。测试挂了要么修代码要么修测试；跳过等于删除。`@skip` / `xtest` / `it.skip` / 注释整段测试代码都不合格。
...
- 测试名必须传达"在什么条件下期望什么行为"的完整命题
- **推荐句式**：`should_<行为>_when_<条件>`（适合 JS / TS / Python 等使用下划线或自由格式的语言）
- 具体语法约束待 ADR-0004 语言落地后细化（Go 用 `TestXxx_<行为>_when_<条件>` 大驼峰、Rust 习惯 `<行为>_when_<条件>` 等）；语言定后本节补对应规范
- 不许 `TestFoo` / `test1` / `it_works` 这类**无信息量**命名——这条与语言无关
- 无信息量命名不合格
...
- 测试名无信息量（`TestFoo` / `test1` / `it_works` / 无完整命题）——与语言无关；具体句式约束按 §"测试命名" 待 ADR-0004 落地后细化
- spec 改动 PR 不含合约测试更新
- 关键路径新增代码无覆盖
