...
- **隐式触发**：被用户纠正后，agent 完成当前纠正之后自检一次。是否沉淀由 agent 判定——**stay quiet by default**，不要自动在每次回复末尾附"沉淀建议"。
- **显式触发**：用户说"沉淀 / 反思 / refine / 复盘 / 记下来"时（不论是否在"被纠正"语境），agent **必须**走自检 4 步。

### 自检 4 步

1. **真问题吗**：是一次性的当下错误，还是有跨 session 复发风险的模式？只有后者值得沉淀。
2. **沉淀到哪层**：见下方分流矩阵。
3. **走对应执行约束**：仓库内任何文件改动遵守核心原则 1（分支先行）；写 auto-memory 遵守全局 `~/.claude/CLAUDE.md` 的"记忆"节；auto-memory 的具体路径与文件命名按各 harness 约定。
4. **复核与落位**：执行前检索同主题条目——auto-memory 层用本 harness 的检索方式，`docs/` 层用 `grep`。命中时判断**补充修订**（已有条目接近，加 bullet 或更新 Why）、**覆盖**（已有条目明显错）、还是**新开**（主题独立）。拒绝盲目新建造成重复。

### 分流矩阵

**跨层判据**（区分个人 scope vs 项目 scope 的统一问题）：
...
同一条经验可能两义皆可——此时走"是"。auto-memory 是**最窄 scope**，没把握就别私藏。

| 经验性质 | 沉淀层 | 是否需 PR |
|---|---|---|
| 用户个人偏好 / 身份锚点 / **对本 agent 的**协作反馈 / 当前项目状态 / 外部系统指针 | auto-memory（Claude Code 下位于 `~/.claude/projects/<slug>/memory/` + `MEMORY.md` 索引；其他 harness 参照自身 memory 文档） | 否 |
| 项目流程规则 / 编码与错误标准 / 对项目协作流程的反馈 | `docs/dev/process/` 或 `docs/dev/standards/` | 是 |
| 架构级决策 | `docs/dev/adr/NNNN-*.md`（新 ADR） | 是 |
| 接口契约 | `docs/dev/spec/**/*.md` | 是 |
| 协作性 skill 新增 / 修改 | `skills/**` + `skills.manifest`（按 [ADR-0007](docs/dev/adr/0007-collaborative-skill-promotion.md)） | 是 |

**误归位红线**：

- 个人偏好写进 `docs/` → 污染 reviewer / 其他协作者
- 项目规则写进 auto-memory → 只活在本地、他人无感
- 架构级决策只进 `process/` 不起 ADR → 不可追溯

### 反模式

- 自动越权改文件（仓库 / memory 都禁；沉淀到有 PR 必要的层必须经用户确认）
- 在 `main` 上直接改 docs（违反核心原则 1）
- 写 memory 时保留会话级溯源（session id / jsonl 路径 / "本 session" 措辞——全局 `~/.claude/CLAUDE.md` 已禁）
- 一个 PR 打包多个沉淀（违反核心原则 7 范围收敛）
- 把 agent 能自决的细节当成"待沉淀经验"塞进 memory（噪音）
