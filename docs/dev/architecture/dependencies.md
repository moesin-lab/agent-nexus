...
- **daemon** 是中枢（hub），只依赖 protocol + 语言标准库 + 经准入的通用基础库；准入标准见 [`dependencies.md`](../standards/dependencies.md)。daemon 不感知具体 platform / agent 实现。
...
依赖准入与边界暴露标准见 [`dependencies.md`](../standards/dependencies.md)。本架构文档只约束 package 间组合方向。
...
| platform-lark | `packages/platform/lark/` | `@agent-nexus/platform-lark` | 已实现；通过官方 Node SDK 接入中国版飞书控制面与话题 Session 文本 |
