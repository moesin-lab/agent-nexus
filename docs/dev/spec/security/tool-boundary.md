...
- 传递方式见 [`claude-code-cli-contract.md`](../agent-backends/claude-code-cli.md) §"启动命令模板" 的 `--cwd` 参数
- 如果 CC 配置允许多个 allowed dirs，沿用 CC 的 allowlist（本项目不重复实现）
- **不继承** agent-nexus 进程的 cwd；每 session 显式传
...
- **`--cwd` 正确传递**：fake CC spawn 后的 argv 中必含 `--cwd <workingDir>`
