...
type: process
status: active
summary: 派发探索 / 侦察类独立 agent 时在 prompt 末尾贴的硬约束片段，用于压制叙述化回报、防止当前 session 上下文腐烂
...
**适用**：探索 / 侦察 / 研究类独立 agent（如 Claude Code 的 Explore / general-purpose 等；其他 harness 参考自身对等），目标是把仓库事实、代码位置、文档片段抓回调用方 session。

**不适用**：Plan（设计类）/ code review（判断类）/ code-simplifier（修改类）——产出结构不同，需要各自的模板。

## 为什么要硬约束

探索类 agent 默认产出叙述化长报告。调用方 session 吞下后会有三类腐烂：

1. **上下文被占满**——后续任务的压缩率下降
2. **真事实埋在叙述里**——调用方 session 还要二次提炼才能用
3. **二手信息污染决策**——调用方 session 误把 agent 摘要当一手事实，实施时才发现要再 Read 原文
...
这与 [`subagent-usage.md`](subagent-usage.md) §反模式 的"派发后当前 session 又做一遍"重复。模板压缩了 agent 回报体积，不代表要失去对 agent 的信任——报告回来后只做收敛，不做复查式重搜。
