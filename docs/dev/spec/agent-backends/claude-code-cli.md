...
4. 启用工具隔离时，跑一次临时 PreToolUse hook deny：向 CC 发起白名单外工具调用请求，断言 hook deny 生效（副作用未发生、对应 `user / tool_result` 的 `is_error:true`、`permission_denials` 非空）；哨兵工具与临时目录由实现决定，但 probe 目录不得含会拦同一哨兵工具的 project/user settings deny，避免把 CC 原生 deny 误判成 hook deny
