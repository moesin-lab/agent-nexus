...
- 新增固定 Codex 0.146.0 app-server 契约的结构化 backend，并提供默认关闭、独立鉴权的只读 tmux viewer；支持 durable thread resume、受控中断、进程组清理，并把源码与安装包真实 Codex 验证证据绑定到 exact tag commit 的发布门禁。
