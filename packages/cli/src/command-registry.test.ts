...
describe('buildCliCommandRegistrationPlan', () => {
  it('builds a Discord plan with platform commands and a single-agent alias for the bound Codex owner', () => {
    ...
    expect(commandKeys(plan)).toEqual([
      ...
      'nexus-sessions:daemon:sessions:stable',
      'nexus-external-sessions:daemon:external-sessions:stable',
      'nexus-new-thread:daemon:new-thread:stable',
      'nexus-working-dir:daemon:working-dir:stable',
      'nexus-settings:daemon:settings:stable',
      'nexus-queue:daemon:queue:stable',
