...
想法
 ...
 │
 ├─> 9. 独立 agent review（大变更额外跑深度 review）
 │
 ├─> 10. 人类 review（如果有协作者）
 │
...
1. **PR 是 review 的承载窗口**：独立 agent review / 深度 review 当前由作者手动触发（见 `code-review.md`），但 diff 展示、评论、反馈与作者回应、决策记录都挂在 PR 上。直接在 `main` commit 等于把这些都丢掉。
...
| 独立 agent review | [`../standards/code-review.md` §独立 agent review 反馈响应合格条件](../standards/code-review.md#独立-agent-review-反馈响应合格条件) |
...
- 把独立 agent review 当 rubber stamp，不逐条回应
