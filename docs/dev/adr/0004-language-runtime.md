...
type: adr
status: active
summary: 实现语言确定为 TypeScript / Node + pnpm workspaces monorepo；驱动因素是 agent-nexus 演进面 N 大（多 agent 后端 / 多 IM 平台 / web 前端 / VSCode 插件 / MCP / 观测评估工具长期目标），TS 一等公民 SDK 生态 + 跨端代码与类型契约共享 + IO-bound 低并发负载契合 Node event loop——这三项的综合优势超过 Go 在 core 心智 / 单二进制分发上的优势
...
- 测试工具链与 eval 生态
- 与 CC CLI 的互操作难度
- **演进面**：未来接入新 agent 后端 / 新 IM 平台 / **web 前端** / **VSCode 插件** / MCP server / 观测评估工具的边际成本
...
- **优点**：
  ...
  - **VSCode extension API 强制 TS/JS**——TS 单语言下 extension 可直接 `import` core 模块；其他语言必走 thin shell + IPC
...
- **VSCode extension 退化为 thin shell + IPC**——TS 全栈下 extension 可直接 `import` core 模块（比 thin shell + IPC 更顺）
...
**关键洞察**：解耦目标在 TS monorepo + 独立 packages 已可达——`@agent-nexus/protocol`（仅类型 + JSON schema 权威源）+ `@agent-nexus/core`（daemon 独立可运行）+ `@agent-nexus/vscode`、`@agent-nexus/web`（独立 UI packages，通过协议跟 daemon 通信，类型 `import` 共享）。物理进程边界（业务-UI 解耦）+ 类型契约共享（同语言 `import`）= 解耦目标全达，**没有跨语言 codegen 税**。
...
| **VSCode extension 集成** | **★★★（直接 `import` core）** | ★（thin shell + IPC） | ★（同 Go） |
...
基于三大契约（platform-adapter / agent-runtime / message-protocol）+ 横切四件套 反观语言适配性，初步倾向 Go。原结论的关键论据：
...
**修正声明（2026-04-25 第三/四轮）**：第二轮的隐含前提是"agent-nexus 演进面 N 偏小 + 不做跨端 UI"。这两个隐含前提在第三 / 四轮被项目 owner 显式纠正：

- N 大：多 backend / 多 IM / MCP / 观测评估工具是长期目标
- 跨端：web 前端 + VSCode 插件是长期目标

第二轮论据 #1（Anthropic SDK 优势"对本项目几乎无效"）仅对 N 小成立；论据 #3（Go 单二进制分发优势）在 web / VSCode 演进下被稀释。
...
项目 owner 在 argue 后显式纠正隐含前提：**演进面 N 大**——多 agent 后端 / 多 IM 平台 / MCP server / 观测评估工具是长期目标。
...
每次新接入 TS 节省 10–30% 时间；累积到 N 大场景，超过 Go 在 core 一次性心智优势。
...
- VSCode extension API 强制 TS / JS——其他语言走 thin shell + IPC
- web 前端事实标准 TS 生态
- TS monorepo（pnpm workspaces）下 daemon / web / VSCode / CLI 可共享 `@agent-nexus/protocol` package（类型契约单一权威源），零 codegen 税
- 任何非 TS 路径都必须为 web / VSCode 这一层付出跨语言协议化成本（手维护双份契约或引入 protobuf / OpenAPI / TypeSpec 工具链）
...
**这是 IO-bound + 低并发数量级（量级 10）+ 单用户负载——正是 Node event loop 的设计中心**。Go goroutine + channel 的并发优势在并发数量 > 1000 或 CPU-bound 多核场景才显现，agent-nexus 全不命中。第二轮"并发 / 长连接 Go ★★★ vs TS ★★"评分修正为"两者均 ★★★"——Go 的并发优势是 idiom 风格优势，不是能力优势，在本项目实际负载下不可观测。
...
剔除被纠正的论据（第二轮的 #1 和 #3）+ 实证修正的论据（并发不构成 Go 优势）后，Go 的剩余真实优势：

- 单二进制分发：在不做 web/VSCode 时真实，在跨端演进下被稀释
- 子进程 / 信号 idiom 顺手：真实但权重小（TS 用 `execa` 可压住）
- 工程化简单（gofmt / Go 1 兼容承诺）：与 N 大演进面无关
- AOT 性能：agent-nexus 不命中需求

Go 的相对优势权重明显下降；TS 的相对优势（SDK 累积 + 跨端共享 + 类型表达力）权重明显上升。**最终推荐：Option A (TypeScript / Node)**。
...
1. agent-nexus 演进面 N 大（多 agent 后端 / 多 IM 平台 / MCP server / 观测评估工具是长期目标）→ TS SDK 一等公民生态的累积优势
2. web + VSCode 跨端 UI 是长期目标 → TS monorepo 下 `@agent-nexus/protocol` 单一权威源，零 codegen 税
3. agent-nexus core 是 IO-bound + 低并发数量级（量级 10）负载——Node event loop 的 sweet spot；Go 并发优势在本项目不显现
4. 解耦目标（业务-UI 独立演进、多客户端复用）在 TS monorepo + 独立 packages 已可达——无需跨语言混合
5. 用户机器 Node 18+ 已被 CC CLI 自身要求满足，分发门槛已被消化
6. 与 CC CLI 同栈红利长期存在（未来可能直接 `import` CC 模块）

## Consequences

- core 子进程 / pty / 信号 / 长连接重连 / FIFO 队列要用成熟库压住心智：`execa` + `node-pty` + `p-queue` + discord.js 自带 ws 重连
...
- monorepo 切分锁定（见 §"采纳后的执行补丁清单" TS-P7）：`@agent-nexus/protocol`（仅类型 + JSON schema 权威源）/ `@agent-nexus/core`（daemon 实现）/ `@agent-nexus/vscode`（extension）/ `@agent-nexus/web`（web frontend）/ `@agent-nexus/cli`（CLI 入口）
...
- discordgo 在 Go 路径下"自补协议字段"的累积成本被消除——直接 npm install 拿 discord.js 最新版
- 多 agent 后端接入边际成本被消除——`pnpm add @anthropic-ai/sdk` / `openai` / `@google/genai` 等直接 import
...
├── core/              # daemon 实现：core + agent + platform；可独立 npm 启动
