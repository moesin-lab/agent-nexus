...
4. `msg.author.id ∉ allowedUserIds` → 丢弃（见 §"用户白名单"——`allowedUserIds` 同时管 inbound chat 与 slash command；空列表 = 拒绝所有）
...
[`config-routing.md`](config-routing.md)；router 不得新增 user allowlist 语义。
...
| Slash command（`/reply-mode`） | ephemeral ack 一句固定文案含调用方 user id；行为见 §"授权" |
...
- CLI loader **必填**该字段：`config.json` 缺 `discord.allowedUserIds` → `ConfigError` 启动失败，错误消息明示要填啥
...
Adapter 在 `start()` 阶段向 Discord 注册一个 slash command：
...
由 config `discord.testGuildId`（可选 string）决定：

| `testGuildId` | 注册作用域 | 生效延迟 | 适用场景 |
|---|---|---|---|
| 缺省 / 空 | **全局**（global）——所有 bot 加入的 guild 可见 | Discord client 缓存最长 1 小时 | 生产形态 |
...
> **为什么暴露这个开关**：global slash command 首次注册到 Discord client 端缓存可见有最长 1 小时延迟，dev 改一次等 1 小时不可接受。per-guild 是 Discord bot 开发的标准 dev workflow。生产部署应**不**配 `testGuildId`，跑 global。
