...
依赖方向见 [`dependencies.md`](dependencies.md)：
...
- **`@agent-nexus/daemon`**（中枢）：引擎 + 横切能力。是 hub，只依赖语言标准库与少量通用工具，不感知具体 agent / platform 实现。
- **`@agent-nexus/agent-<name>`**（agent 适配器）：具体 agent 后端实现。当前实现有 `agent-claudecode`，Codex 后端由 ADR-0014 / `agent-backends/codex-cli.md` 先行锁定契约后接入；具体启用哪个由 CLI 配置选择。
- **`@agent-nexus/platform-<name>`**（IM/transport 适配器）：具体 IM 平台或 transport 实现。当前只有 `platform-discord`，通过注册表接入 daemon。
- **`@agent-nexus/cli`**（拼装入口）：可执行入口，拼装 daemon + 启用的 agent / platform，加载配置。
...
2. **横切能力（观测、幂等、限流、脱敏）由 `@agent-nexus/daemon` 强制提供，** 不允许 platform/agent 自己实现一套。
3. **package import 方向**见 [`dependencies.md` §允许方向 / §禁止方向](dependencies.md#允许方向)（owner）。本 overview 不复述。
...
daemon.Engine.dispatch(NormalizedEvent)
  ...
  - 路由到对应 session 的 FIFO 队列
        │
        ▼
agent/<selected>.SendInput(session, input)
  - 管理所选 backend 的子进程（spawn / reuse / resume）
  - 写入 stdin，启动/恢复会话
        │
        ▼
backend CLI 产生输出事件流（stdout）
```

### 出站（agent → 用户）

```
backend CLI stdout 流
        │
        ▼
agent/<selected>
  - 解析 stdout 为标准化 AgentEvent（thinking / tool_call / text_delta / final；按 backend contract 支持能力）
        │
        ▼
daemon.Engine 聚合
  - 记账（token、成本）
