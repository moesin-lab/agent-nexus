...
`TrajectoryObservabilityConfig` 嵌入解析后的 `DaemonRuntimeConfig`，字段归 daemon owner 校验；配置文件可省略子对象，parser 用下列默认值补齐。
...
| `providerCapture.retentionDays` | provider observation 保留天数；按该字段清理 `provider_call_observations` 及对应 `provider-call` segments |

当前配置面不包含 imported segment 自动清理字段；`daemon.trajectory.retention.*` 不是合法配置。
...
interface ExternalSessionSourceAdapter {
    ...
    importCandidate(candidate: ExternalSessionCandidate, policy: ExternalImportPolicy) -> ImportedTranscriptSegment[]
