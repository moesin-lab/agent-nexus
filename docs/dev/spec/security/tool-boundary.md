...
- Claude Code CLI 可用工具集由 `SessionConfig.toolWhitelist` 控制（见 [`agent-runtime.md`](../agent-runtime.md) §SessionConfig）
...
- `SessionConfig.toolWhitelist` 不得被解释为 Codex native allowlist。
- `AgentCapabilitySet.supportsNativeToolWhitelist` 必须为 `false`。
...
- **白名单外拒绝（执行前）**：CC 尝试调用未在 `toolWhitelist` 的工具 → `can_use_tool` control 强制点在工具**执行前** deny；**最低断言 = 工具副作用未发生（执行前被拦）**。具体可观测信号（stdout 格式 / denial 汇总结构）由 [`claude-code-cli.md`](../agent-backends/claude-code-cli.md) 拥有。测试不得只断言有 denial 文本，必须同时验证副作用未发生。若实现切到 PreToolUse hook fallback，同一最低断言仍成立
...
- **Codex native whitelist 不支持**：Codex backend 的 capability 为 false；构造 runtime 不得把 `toolWhitelist` 翻译成不存在的 allowlist flag
