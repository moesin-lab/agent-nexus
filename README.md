...
- 配置、密钥和运行状态默认放在 `~/.agent-nexus/`，也可用 `--home` 或 `AGENT_NEXUS_HOME` 跑多实例；本仓库约定 `~/.agent-nexus` 用作 dev 实例，`~/.agent-nexus-stable` 用作 stable 实例。
...
需要跑 stable / dev 等多个实例时，用 `--home <dir>` 或 `AGENT_NEXUS_HOME=<dir>` 指定实例根目录；`config.json`、`secrets/`、`state/` 都会从这个根目录派生。本仓库约定 dev 使用 `~/.agent-nexus`，stable 使用 `~/.agent-nexus-stable`。
