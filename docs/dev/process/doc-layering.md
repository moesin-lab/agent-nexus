...
| 层 | 唯一回答的问题 | 准入清单 | 禁入清单 |
|---|---|---|---|
| **ADR**（`docs/dev/adr/`） | **为什么**选 X 不选 Y | 选项对比、权衡、约束、放弃理由、决策日期 | 接口签名、数据结构定义、字段表、默认值字面量、操作流程、规则清单本体 |
| **spec**（`docs/dev/spec/`） | X **是什么** / X **长什么样** | 接口签名、数据结构、字段表、默认阈值、错误码、事件类型、契约不变量 | 决策权衡论述（"因为 ... 所以选这个"）、模块拓扑图、跨模块组合关系 |
| **architecture**（`docs/dev/architecture/`） | X、Y、Z **怎么组合** | 模块名、依赖关系、数据流、状态转换图、横切能力索引、跨模块 link | 接口签名、字段表、默认值字面量、决策权衡论述 |
