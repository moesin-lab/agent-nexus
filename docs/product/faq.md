...
支持，但默认 backend 仍是 Claude Code。要启用 Codex，把 `agent.backend` 设为 `codex`，并填写 `codex.workingDir`。Codex backend 使用 `codex exec --json` / `codex exec resume`。启动时默认只跑快速 compatibility probe：检查 `codex --version` 与 help 里是否有必需 flag，不发起真实模型 turn；本机未安装 Codex CLI 或 CLI 形态不匹配时会拒绝启动。

Codex CLI 当前没有 Claude Code 那种执行前工具审批。它的边界来自 `codex.sandbox`、`codex.addDirs`、`--ask-for-approval never`、工作目录和默认不加载用户全局 config / rules。
