...
- **通用 vs backend 私有**：跨 backend 的通用事件 `agent_*` 前缀进本清单；某个 backend 实现内部独有的事件以 `<backend>_` 前缀（如 `claudecode_*`、`codex_*`）命名，由对应 backend 的 contract spec 登记，不进本清单。implementation 不得自创 spec / contract 未登记的事件名（见 [`../../standards/logging.md`](../../standards/logging.md)）。
...
| `agent_spawn_started` | 启动 agent 后端 | `pid?` |
| `agent_spawn_succeeded` | agent 后端就绪 | `pid`, `latencyMs` |
| `agent_spawn_failed` | agent 后端启动失败 | `errorKind`, `cause` |
| `agent_compat_probe_failed` | agent 后端兼容性自检失败 | `agentBackend`, `errorKind`, `code`, `cause`, `stack?` |
| `agent_input_sent` | 发输入给 agent 后端 | `inputType` |
| `agent_event_received` | 收到 agent 后端事件 | `agentEventType`, `sequence` |
| `llm_call_started` | LLM 调用开始 | `model` |
| `llm_call_finished` | LLM 调用结束 | 完整字段见 §"LLM 调用事件必含字段" |
| `tool_call_started` | 工具调用 | `toolName`, `callId` |
| `tool_call_input` | 工具输入摘要（trace） | `toolName`, `callId`, `inputSummary` |
| `tool_result` | 工具结果（trace） | `callId`, `resultSequence`, `isError`, `content` |
...
| `external_session_discovered` | 外部 session discovery 找到 candidate | `sourceAdapter`, `sourceSessionId`, `confidence`, `unsupportedReasons` |
| `external_session_imported` | 外部 transcript segment 写入 trajectory read model | `importId`, `sourceAdapter`, `segmentCount`, `confidence` |
| `external_session_resume_registered` | 外部 session native ref 绑定到 RoutingSession | `importId`, `sessionId`, `agentBackend`, `confidence` |
| `trajectory_segment_recorded` | trajectory segment 写入 | `segmentId`, `sessionId?`, `source`, `kind`, `redactionState` |
| `provider_capture_started` | opt-in provider capture 启动 | `agentBackend`, `captureMode`, `bindHost`, `port?` |
| `provider_call_observed` | provider/request-level observation 写入 | `observationId`, `sessionId?`, `agentBackend`, `captureMode`, `alignmentConfidence`, `redactionState` |
| `provider_call_dropped` | provider observation payload 因策略或 redaction 被丢弃 | `agentBackend`, `captureMode`, `reason`, `code?` |
| `provider_capture_failed` | provider capture 初始化或运行失败 | `agentBackend`, `captureMode`, `errorKind`, `code`, `cause` |
...
| `latencyMs` | int | 是 | |
| `costUsd` | float \| null | 否 | backend 返回的美元金额，经解析层归一化（见 [`agent-backends/claude-code-cli.md` §UsageCompleteness](../agent-backends/claude-code-cli.md#usagecompleteness)）；只能是有限非负数或 null；订阅 / Max plan 下可能为 0 或 null。是否可用于美元决策见 `completeness` |
...
| `completeness` | string | 是 | `complete` / `partial` / `missing`（语义见 [`cost-and-limits.md` §`UsageRecord.completeness` 语义](cost-and-limits.md#usagerecordcompleteness-语义)） |
...
`level: warn` 的 error-like 事件（非中断但与错误归因相关，如 backend 子进程非零退出后仍能继续的诊断告警）允许带 `errorKind` / `code` / `cause`，字段语义与 error 级一致；warn 不要求 `stack`。
