...
三个 agent backend、Discord platform 和 Lark platform 八个 workspace package 组成。内部包用于
...
CLI bundle 已把七个内部 workspace package 编入单个 JavaScript 入口。第三方原生依赖
...
- **是什么**：只发布 `@moesin-lab/agent-nexus`，提供 `agent-nexus` bin；七个内部包只参与构建并进入 bundle。
- **优点**：
  - 用户只安装一个包，符合当前产品入口。
...
- **是什么**：CLI 与七个内部包全部改为 public，并用 workspace 版本关系发布。
- **优点**：
...
选择 Option A：首发只发布 `@moesin-lab/agent-nexus` CLI，七个内部 workspace package
...
- 2026-08-20：新增 private `@agent-nexus/agent-codex-app-server` backend 后，workspace 总数由七增至八、bundle 内部包由六增至七；单一公开 CLI 的决策不变。
