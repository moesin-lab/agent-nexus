...
> ⚠️ **硬性要求**：本节列的工具是本 skill 在 Claude Code 下的必走路径——`codex-review` skill、`general-purpose` Agent 派发、`AskUserQuestion` 原生工具等。agent **不得静默降级**为普通对话文本、手工模拟提问、自问自答假 argue 等通用做法。若某工具当前不可用（权限 / 配额 / 未安装），**在 PR 里显式声明并与用户确认兜底方案**，不装作走了特色路径。
