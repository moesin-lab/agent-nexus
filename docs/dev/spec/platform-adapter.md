...
`settingsSnapshot` 是只读、按调用用户求值的反向读 port，用于 daemon 拼装 `/nexus-settings` 面板。它只能暴露平台 owner 自己持有的设置，例如 Discord adapter 的 reply-mode；daemon-owned session / thread / workingDir 仍由 daemon 自己拼入 snapshot。每个 item 必须带 `owner`、`source`、`durability` 与 `canChange`，其中 `canChange` 使用调用方身份判断，不能把平台 allowlist 逻辑搬到 daemon 里重写。
...
| `threadParentChannelId` | 否 | thread 消息所属父 channel；daemon 可用于 thread 会话继承父频道授权 |
...
`EventHandler` 可返回 `EventHandlerResult.commandResponse`，仅用于已被 adapter native defer/ack 的 command / component interaction 收尾。Discord adapter 对非 reply-mode slash command 与 message component interaction 先发 ephemeral deferred reply；daemon 若返回 `commandResponse.text`，adapter 必须用 `editReply` 展示该 ephemeral 反馈，否则清理 deferred reply。`commandResponse.components` 可携带平台中立的按钮或 string select；Discord adapter 映射为 Action Row + Button / String Select。普通消息与 agent 成功输出仍走 `OutboundMessage`。

`/nexus-settings` 默认复用上述 deferred ephemeral 路径：每次 slash command 或组件 action 都返回一条新的 ephemeral 结果与最新 snapshot，不使用 Discord `deferUpdate` 或 `interaction.update()`。需要打开 modal 的 button 必须在 `deferReply` 前调用 daemon handler，若返回 `modalResponse` 则立即 `showModal`；当前包括 `nexus:settings:working-dir`、`nexus:queue:insert` 与 `nexus:queue:edit:<itemId>`。modal submit 再走普通 deferred ephemeral 回显。其它需要首响应分类的能力必须另行扩展 adapter ack 契约。
...
CapabilitySet {
    ...
    supportsStringSelects: bool
    supportsThreads: bool              // 是否支持 thread 作为 channelId
    supportsThreadCreation: bool        // 是否支持 daemon 主动创建 thread
...
| Button click | `NormalizedEvent { type: "interaction", interaction: { customId, componentType: "button", values: [] } }` |
| String select | `NormalizedEvent { type: "interaction", interaction: { customId, componentType: "string-select", values } }` |
...
`statePath` 必填且 adapter 不自己选位置——避免 adapter 在文件系统里乱建目录。多 platform instance 配置下，CLI 必须传入实例级路径，例如 `~/.agent-nexus/state/platforms/<platformName>.json`，避免多个 Discord bot 共用 reply-mode 状态；目录权限 `0700` 与 secrets 一致。
...
Component {
    type: "button" | "string-select"
    customId: string                  // Discord custom_id；1-100 chars
    label: string?                    // button 必填；Discord button label max 80 chars
    style: "primary" | "secondary" | "success" | "danger"?
    placeholder: string?              // string-select
    options: { label, value, description?, default? }[]?
    minValues: int?
    maxValues: int?
    disabled: bool?
}
