...
`llm_call_finished` 的字段 = `AgentEvent{type: usage}` 的 `UsageRecord`（见 [`agent-runtime.md`](../agent-runtime.md) §UsageRecord）+ 日志通用字段（`traceId` / `sessionId` / `sessionKey` 等）。`core.counters` 收到 `usage` 事件后**原样**转写为 `llm_call_finished` 结构化日志，**不发明新字段名**。
...
- 定价表由 core 维护，支持热更新（配置文件）；仅 `$ 预算 opt-in 启用`时生效。
...
脱敏在日志 sink 入口强制执行（见 [`redaction.md`](../security/redaction.md)）。
