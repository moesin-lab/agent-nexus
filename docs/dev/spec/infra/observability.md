...
| `latencyMs` | int | 是 | |
| `costUsd` | float \| null | 否 | 按模型定价表计算；订阅模式下 backend 可能不返回，留 null |
...
| `completeness` | string | 是 | `complete` / `partial` / `missing`（见 [`agent-runtime.md`](../agent-runtime.md) §UsageRecord） |
