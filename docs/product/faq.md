...
支持，但默认 backend 仍是 Claude Code。要启用 Codex，把 `agent.backend` 设为 `codex`，并填写 `codex.workingDir`。Codex backend 使用 `codex exec --json` / `codex exec resume`，启动时会跑 compatibility probe；本机未安装或未登录 Codex CLI 时会拒绝启动。
