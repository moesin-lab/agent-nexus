...
- 2026-04-25：命名澄清（非决策反转）——TS-P7 的 npm package 名由 `@agent-nexus/core` 修订为 `@agent-nexus/daemon`，理由 = package 概念范围（含核心引擎 + 横切 + 多种 agent / platform 实现）大于 architecture `core` 模块概念范围，应反映"daemon 进程整体"语义而非借用最小子模块的名字
- 2026-04-25：同步修订 architecture/spec 文档中带点的 namespace `core.xxx` → `daemon.xxx`；模块概念维度的散文 / ASCII 路径 `core` 保留；monorepo + TS + 5 包结构不变
- 2026-04-25：废止架构语境下 "三层结构 / layered architecture" 措辞——agent-nexus 实际是 hub-and-spoke（中枢辐射）模型，与 LSP / DAP / MCP / Continue.dev 等权威对标使用 client/server/adapter/extension 等角色名一致；旧版 architecture/overview.md 的"三层结构"章节归档到 `docs/_deprecated/architecture/three-layer-vocabulary.md`
- 2026-04-25：packages 结构重构——TS-P7 由 5 包扩为 7 包，platform/agent 从 daemon 内部子目录提升为顶级独立 package（`packages/platform/<name>/` 与 `packages/agent/<name>/` 嵌套子目录，npm name 带前缀 `@agent-nexus/platform-<name>` / `@agent-nexus/agent-<name>`）；理由 = 与权威对标 Continue.dev `core/+binary/+extensions/+packages/openai-adapters/` 模式对齐 + 第三方 transfer adapter 可独立 publish + 横切能力强制走 daemon import + daemon 单一职责（核心引擎 + 横切，不含任何 transfer 实现）；接口契约（PlatformAdapter / AgentRuntime）从 daemon 移到 protocol package
...
```
packages/
├── protocol/                  # 类型 + 接口契约（PlatformAdapter / AgentRuntime / NormalizedEvent / AgentEvent / OutboundMessage / SessionConfig 等单一权威源）
├── daemon/                    # 核心引擎 + 横切（auth / idempotency / sessions / ratelimit / counters / redact / persistence / observability）；不含任何 platform / agent 实现
├── platform/
│   └── discord/               # Discord 协议转换层（实现 @agent-nexus/protocol 的 PlatformAdapter 接口）
├── agent/
...
- 物理目录：`packages/platform/<name>/` / `packages/agent/<name>/`（嵌套子目录）
- npm name：`@agent-nexus/platform-<name>` / `@agent-nexus/agent-<name>`（带前缀避免和外部 npm 包重名）
- pnpm-workspace.yaml：
  ```yaml
  packages:
    - 'packages/*'
...
- 依赖方向：
  - `protocol`：无依赖（leaf）
  - `daemon` → `protocol`
  - `platform/*` / `agent/*` → `daemon`, `protocol`
  - `vscode` / `web` → `protocol`（仅类型，IPC schema 共享）；运行时通过 stdio/WebSocket 接 daemon 进程
  - `cli` → `daemon`, `platform/*`, `agent/*`, `protocol`（唯一拼装层；拼装时按配置选择启用哪些 transfer）

**关键设计点**：

1. **接口契约住 protocol**：`PlatformAdapter` / `AgentRuntime` 接口在 `@agent-nexus/protocol` 定义，transfer 实现包仅 import protocol 拿接口签名，仅在需要核心横切时才 import daemon——降低 transfer 包对 daemon 的耦合
2. **横切能力强制走 daemon**：transfer 想用 logger / auth / idempotency / redact 必须 import `@agent-nexus/daemon`——和 cc-connect 的"core 强制提供横切"教训对齐，物理上无法绕过
3. **第三方 transfer 可独立贡献**：未来 community 贡献 `@agent-nexus/platform-slack` / `@agent-nexus/agent-openai-cli` 是 npm package 级别的，不需要 fork daemon
4. **daemon 单一职责**：核心引擎 + 横切，不感知具体 IM 平台或 agent 后端
5. **"打包成一个二进制" = 单进程多 package**：pnpm workspace 同进程 import，不是跨进程 IPC——不命中本 ADR §Form D-1 已驳回的"跨进程 IPC"反 alternative

> **命名 disambiguation**：本仓库中 `core` 一词专指 architecture 中枢模块（见 [`docs/dev/architecture/overview.md`](../architecture/overview.md) §模块结构）；package 名一律不再使用 `core`。文档里带点的 namespace prefix（`daemon.logger` / `daemon.idempotency` 等）= `@agent-nexus/daemon` 的 import path。本项目不使用"三层结构 / layered architecture"措辞，旧版表述已归档（详见 [`docs/_deprecated/architecture/three-layer-vocabulary.md`](../../_deprecated/architecture/three-layer-vocabulary.md)）。
